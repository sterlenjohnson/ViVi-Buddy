...
import { GpuUsageDetail } from '../utils/calculations';
...
const StackedMemoryBar: React.FC<StackedMemoryBarProps> = ({ label, segments, maxValue, showPercent = false }) => {
            ...
            <div className="flex gap-2 mt-1 text-[10px] text-slate-400 flex-wrap">
                {segments.map((seg, idx) => seg.value > 0 && (
                    <div key={idx} className="flex items-center gap-1">
                        <div className="w-1.5 h-1.5 rounded-full" style={{ backgroundColor: (colorMap as any)[seg.color] }} />
                        <span>{seg.label}</span>
                    </div>
                ))}
            </div>
...
interface Calculations {
    gpuUsageDetails: GpuUsageDetail[];
    cpuWeights: number;
    overhead: number;
    ...
    vramOverflowPenalty: number;
    ramOverflowPenalty: number;
...
const ResultsPanel: React.FC<ResultsPanelProps> = ({ calculations, gpuList, systemRAMAmount, isUnified, cpuCores, cpuThreads, gpuEnabled }) => {
